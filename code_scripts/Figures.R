...
#high and low predictability years
...
#grab index of pre, post, and observed years 
#762 - 1489
...
#temp: leesferry - long scatterplots 
...
for(j in 1:4){
  ...
  my.plot<-scatterplot3d(embb[,1],embb[,2],embb[,3],type="l",xlab=xlb ,ylab=ylb,zlab=zlb,color="gray",lwd=2,axis = TRUE, box = FALSE, main = paste(indexs[i],'-', indexs[i]+39, sep=" "))
...
#temp: predictability for all locations 
epoch = 1:12
...
#temp: predictability windows only for each gauge
...
for(name in names){
  ...
  #temp: figure with avgLLE, rolling average and rolling variance with windows
